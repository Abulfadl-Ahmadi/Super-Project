...
\newcommand{\test}[1]{\textbf{\item {\large #1}}}
\newsavebox{\answera}
\newsavebox{\answerb}
\newsavebox{\answerc}
\newsavebox{\answerd}
...
\begin{enumerate}[label={\textbf{\arabic*-}}]
	...
	\test{
دو دانش‌آموز، هم‌زمان شروع به حل معادلهٔ
\riazi{2x^2 + ax +b =0}
می‌کنند. اولی جواب‌ها را ۳ و ۶ و دومی جواب‌ها را ۲ و ۵ به دست آورده است. اگر بدانیم اولی عدد ثابت معادله و دومی ضریب 
\riazi{x}
را اشتباه دیده‌اند، تفاضل ریشه‌ها در صورت صحیح معادله کدام است؟	
}\answer{}{}{}{}
	%\quenstion{206}
	\quenstion{207}
	...
	\test{
		...
		در معادلهٔ درجه دوم
		\riazi{(m+1)x^2-3x+m}
		یکی از ریشه‌ها دو برابر ریشه‌ٔ دیگر است؟
}\answer{$3,-2$}{$2,-3$}{$2,-1$}{$1,-2$}
	%\quenstion{208}
	\quenstion{209}
...
\riazi{\alpha} و \riazi{\beta}
ریشه‌های معادلهٔ
\riazi{(2m+1)x^2-(4m+2)x+(m+1)-0}
باشد و بین آن‌ها رابطهٔ
\riazi{5\alpha +\beta \alpha - 2\beta^2 =20}
برقرار باشد،
 مقدار \riazi{m} کدام است؟
}\answer{1}{2}{3}{4}

	
	\test{
معادلهٔ 
\riazi{\dfrac{1}{x^2+x-4} +\dfrac{1}{x^2+x+1} + \dfrac{1}{x^2+x+4} =0}،
چند جواب دارد؟
}\answer{صفر}{۲}{۳}{۴}
	
	
	\test{
تعداد ریشه‌های حقیقی و متمایز معادلهٔ
\riazi{(x+1)(x+2)(x+6)(x+7)=36}
کدام است؟	
}\answer{1}{2}{3}{صفر}
...
\riazi{x^2-x-1=0}
باشند، حاصل عبارت
\riazi{\left(\alpha + \dfrac{1}{\alpha -1}\right)^2 + \left(\beta + \dfrac{1}{\beta -1}\right)^2}
کدام است؟
}\answer{$4\sqrt{5}$}{4+4$\sqrt{5}$}{8}{12}
	%\quenstion{223}
...
یک ریشهٔ معادلهٔ
\riazi{3x^2-ax+4=0}،
سه برابر ریشهٔ دیگر است. اختلاف این دو مقدار
\riazi{a}،
کدام است؟	
}\answer{8}{9}{16}{18}
	%\quenstion{224}	
...
\riazi{\alpha} و \riazi{\beta}
ریشه‌های معادلهٔ
\riazi{x^2+2(a+1)x+2a-1=0}
باشند، به ازای کدام مقدار 
\riazi{a}،
به ترتیب سه عدد 
\riazi{\alpha}، \riazi{a} و \riazi{\beta}
...
\riazi{\alpha < \beta < 0}
و
\riazi{3\alpha^2 + 2\beta^2 = 12\sqrt{2} + 85}
باشد، مقدار \riazi{a} چقدر است؟
}\answer{1}{$\dfrac{13}{4}$}{$\dfrac{21}{5}$}{2}
%\quenstion{226}

\test{
اگر مجموع و حاصل‌ضرب ریشه‌های حقیقی معادلهٔ 
\riazi{x^4-7x^2-5=0}
به ترتیب \riazi{S} و \riazi{P} باشند، حاصل‌ عبارت \hfill
\riazi{2P^2-3SP+2S}،
 کدام است؟
}\answer{$59-7\sqrt{69}$}{$7+\sqrt{69}$}{$50$}{$59+7\sqrt{69}$}
%\quenstion{227}
...
و
\riazi{y=-x^2-2x+b}
مشترک هستند. اگر از دو نقطه با عرض یکسان روی دو سهمی خط
\riazi{y=1}
رسم شود،
\riazi{ab}
چقدر است؟
...
\riazi{15x^2+73x+14}
منفی و عبارت
\riazi{\left| \dfrac{x-1}{2}-1 \right|}
بزرگ‌تر از سه است. بیشترین مقدار
\riazi{b-a}
کدام است؟}

\answer{$\dfrac{5}{3}$}{$\dfrac{23}{3}$}{$\dfrac{4}{15}$}{$\dfrac{67}{15}$}

\test{
تابع
\riazi{f(x)=mx^2-nx-k}
در هر بازه‌ای، هم صعودی و هم نزولی است. اگر مجموعه زیر، تابع باشد، مقدار
\riazi{f(\sqrt5)}
کدام است؟ \hfill
\riazi{\left\{(m, n-1), (0, k), (n-1, m^2+2m-1), (3k+2, 2k+1)\right\}}
}
\answer{$-1$}{$-\sqrt5$}{1}{$\sqrt5$}
...
ریشه‌های معادلهٔ
\riazi{ax^2-8x+4=0}
است. اگر مجموع و حاصل‌ضرب ریشه‌ای معادله‌ای با ریشه‌های
\riazi{\alpha^2\beta}
و
\riazi{\alpha\beta^2}
برابر باشند، مقدار
\riazi{\log{_{\sqrt{2}}a}}
کدام است؟}
...
\test{
معادلهٔ
\riazi{\sqrt{2x-3}=\sqrt{x+\sqrt{x-2}}-\sqrt{2-x}}
چند ریشه حقیقی دارد؟
}
\answer{3}{2}{1}{صفر}
...
زاویهٔ
\riazi{60^\circ}
درجه می‌سازد. اختلاف مقدار
\riazi{m}
کدام است؟
}
\answer{$2\sqrt{3}$}{$4\sqrt{3}$}{$\dfrac{2}{\sqrt3}$}{$\dfrac{4}{\sqrt3}$}

\test{
\riazi{a, b, c}
اعدادی دو به دو متمایز هستند؛ می‌دانیم سه معادلهٔ درجه دوی زیر ریشه‌ای مشترک دارند. مقدار آن ریشهٔ مشترک چقدر؟}

\begin{center}
	$\bm{ax^2+bx+c=0 \quad , \quad bx^2+cx+a=0 \quad , \quad cx^2 + ax+b=0}$
\end{center}


۱) صفر \hfill
۲) $\dfrac{1-\sqrt{5}}{2}$ \hfill
۳) $-1$ \hfill
۴) ۱ \hfill
۵) به‌طور یکتا تعیین نمی‌شود. \sol
...
برای عدد حقیقی 
\riazi{x}،
کم‌ترین مقدار عبارت زیر چند است؟
\begin{center}
	\riazi{\sqrt{x^2+4x+8} + \sqrt{x^2-6x+10}}
\end{center}
}
...
۲) $4\sqrt2$ \hfill
۳) $\sqrt{34}$ \hfill
۴) $\sqrt{5} + \sqrt{13}$ \hfill
۵) $\sqrt{8} + \sqrt{10}$ \hfill
\sol \sol

%\quenstion{313}

\test{
چهار مربع به ضلع ۲ واحد مانند شکل کنار یکدیگر قرار دارند. یک سهمی از نقاط 
\riazi{A}، \riazi{B} و \riazi{C}
عبور کرده و محور 
\riazi{x}ها
را در دو نقطه قطع می‌کند. فاصله نقاط تقاطع چند واحد است؟ 
}
...
\end{minipage}
\begin{minipage}{6cm}
	\includegraphics[width=6cm]{../Source/314}
\end{minipage}
...
\test{
	تعداد جواب‌های صحیح معادله زیر را بیابید
	\begin{center}
		\riazi{(x^2+5x+5)^{(x^2-10x+21)} = 1}
	\end{center}
}
...
\test{
در معادلهٔ درجهٔ سوم 
\riazi{ax^2+bx+c=0}
ضرایب همگی اعداد گویا هستند و می‌دانیم که یکی از ریشه‌های آن با حاصل‌ضرب دو ریشهٔ دیگر برابر است. ثابت کنید همین ریشه، عددی گویاست.
}
